...
def _split_xy(df: pd.DataFrame, label_col: str = "label") -> Tuple[pd.DataFrame, np.ndarray, List[str]]:
...
def train_all(
    ...
    # 1) split y encoding
    X, y, feature_names = _split_xy(df_features, label_col)
    le = LabelEncoder()
    ...
    for name, model in models.items():
        print(f"\n>>> Entrenando {name}…")
        grid = GridSearchCV(model, param_grids[name], cv=cv, n_jobs=-1, scoring="f1_weighted", refit=True)
